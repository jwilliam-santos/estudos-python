...
#Você pode usar uma variável para controlar quando a lista foi alterada (novo, altera, apaga) e reinicializar esse valor quando ela for lida 
#ou gravada.
agenda = []
alterada = False
...
while opção := menu():
    if opção == 0:
        print(f"Tchau, sua alteração foi {alterada}")
        break
    elif opção == 1:
        novo()
    elif opção == 2:
